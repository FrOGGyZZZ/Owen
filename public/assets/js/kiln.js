...
var selected_profile_name = 'Базовый';
var temp_scale = "c";
...
{
console.log("profile:" + selected_profile_name);
if(selected_profile_name === "Базовый")
{
$.bootstrapGrowl("<span class=\"glyphicon glyphicon-exclamation-sign\"></span> <b>ERROR :</b><br/>Базовый удалять не надо=)", {
...
function updateProfileTable()
{
    ...
    var html = '<h3>Точки профиля обжига</h3><div class="table-responsive" style="scroll: none"><table class="table table-striped">';
        html += '<tr><th style="width: 50px">#</th><th>Время от начала в ' + time_scale_long + '</th><th>Температура в °'+temp_scale_display+'</th><th>Ход нагрева &deg;'+temp_scale_display+'/'+time_scale_slope+'</th><th></th></tr>';
    ...
    for(var i=0; i<graph.profile.data.length;i++)
    {
        ...
        html += '<td><div class="input-group"><span class="decrease5 glyphicon glyphicon-minus-sign input-group-addon ds-trend" style="background: #2471A3;"></span><input type="text" class="form-control" id="profiletable-0-'+i+'" value="'+ timeProfileFormatter(graph.profile.data[i][0],true) + '" style="width: 60px" /><span class="increase5 glyphicon glyphicon-plus-sign input-group-addon ds-trend" style="background: #2471A3;"></span></div></td>';
        html += '<td><div class="input-group"><span class="decrease10 glyphicon glyphicon-minus-sign input-group-addon ds-trend" style="background: #CA6F1E;"></span><input type="text" class="form-control" id="profiletable-1-'+i+'" value="'+ graph.profile.data[i][1] + '" style="width: 60px" /><span class="increase10 glyphicon glyphicon-plus-sign input-group-addon ds-trend" style="background: #CA6F1E;"></span></div></td>';
...
let increase5 = $('.increase5');
increase5.on('click', function(){
   $(this).prev().val(parseInt($(this).prev().val()) + 5);
   $(this).prev().change();
})

let decrease5 = $('.decrease5');
decrease5.on('click', function(){
   if ($(this).next().val() > 4)
   {
     $(this).next().val(parseInt($(this).next().val()) - 5);
     $(this).next().change();
   }
})

let increase10 = $('.increase10');
increase10.on('click', function(){
   $(this).prev().val(parseInt($(this).prev().val()) + 10);
   $(this).prev().change();
})

let decrease10 = $('.decrease10');
decrease10.on('click', function(){
   if ($(this).next().val() > 9)
   {
     $(this).next().val(parseInt($(this).next().val()) - 10);
     $(this).next().change();
   }
})
...
function enterNewMode()
{
    ...
    $('#form_profile_name').attr('placeholder', 'Введите название');
...
$(document).ready(function()
{
    ...
    else
    {
        ...
        ws_status.onopen = function()
        {
            ...
            $.bootstrapGrowl("<span class=\"glyphicon glyphicon-exclamation-sign\"></span> <b>Инициализация</b><br/>Дайте минуту получаю все данные!!",
            {
            ele: 'body', // which element to append to
            ...
            if(state!="EDIT")
            {
                ...
                if (state!=state_last)
                {
                    if(state_last == "RUNNING")
                    {
                        $('#target_temp').html('---');
                        updateProgress(0);
                        $.bootstrapGrowl("<b>Обжиг закончен</b>", {
                        ele: 'body', // which element to append to
                        type: 'success', // (null, 'info', 'error', 'success')
                        offset: {from: 'top', amount: 250}, // 'top', or 'bottom'
                        align: 'center', // ('left', 'right', or 'center')
        ...
        ws_config.onmessage = function(e)
        {
            ...
            switch(time_scale_profile){
                case "s":
                    time_scale_long = "Секундах";
                    break;
                case "m":
                    time_scale_long = "Минутах";
                    break;
                case "h":
                    time_scale_long = "Часах";
                    break;
            }
