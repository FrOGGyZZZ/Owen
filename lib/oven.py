...
try:
    if config.max31855 + config.max6675 + config.max31855spi + config.max31856 > 1:
    ...
    if config.max31856:
        from max31856 import max31856, FaultError
        log.info("import MAX31856")
...
class TempSensorReal(TempSensor):
    def __init__(self, time_step):
        ...
        if config.max31856:
            ...
            self.thermocouple = max31856(config.gpio_sensor_cs,
                                     config.gpio_sensor_data_i,
                                     config.gpio_sensor_data,
                                     config.gpio_sensor_clock,
                                     config.max31856_termocouple_type)
